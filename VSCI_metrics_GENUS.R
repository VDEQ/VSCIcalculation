...
metprop_function <- function(x,bugtotals){
  mets <- left_join(metrich(x),metsum(x),by=c('BenSampID','metric'))
...
summaryStress <- function(metprop, metricName, percent, specialName){
  z <- filter_(metprop, interp(~metric == metricName)) %>%
    group_by(BenSampID) 
  if(percent == FALSE){
...
pickTopTwoDom <- function(y){
  ...
  ungroup() %>% 
  group_by(BenSampID) %>%
  arrange(desc(twodtot)) %>% #These two arrange lines put the family in descending order
  arrange(BenSampID) %>%   
  slice(1:2)} #incase there is tie
